...
#define CONN_NAME L"VideoConnect"
        ...
        if (SUCCEEDED(pNewStreamConfig->QueryInterface(IID_IWMPropertyVault, (void**)&strprops))) {
            WORD cplx = (WORD)cfg.cplx;
            if (cplx > codecs[cfg.codecIndex].cmplxMax) { cplx = WORD(codecs[cfg.codecIndex].cmplxMax); }
            strprops->SetProperty(g_wszComplexity, WMT_TYPE_WORD, (BYTE*)&cplx, sizeof(cplx));
            strprops->Release();
        }

        JIF(pProfile->AddStream(pNewStreamConfig));
        pNewStreamConfig->Release();
        pNewStreamConfig = NULL;
        pProfile3->Release();
        pProfile3 = NULL;
    ...
    virtual void __stdcall end(void) {
        ...
        if (pWMWriter) {
            if (writing) { pWMWriter->EndWriting(); }
            writing = false;
            if (filesink) { filesink->Release(); }
            filesink = NULL;
            if (mysink) { mysink->Release(); }
            mysink = NULL;
            if (pWMWriterA) { pWMWriterA->Release(); }
            pWMWriterA = NULL;
            if (pWMWriter) { pWMWriter->Release(); }
            pWMWriter = NULL;
        }
        if (pProfile) { pProfile->Release(); }
        pProfile = NULL;
        if (m_pCodecInfo) { m_pCodecInfo->Release(); }
        m_pCodecInfo = NULL;
    }
private:
    struct Tcontext {
    ...
    bool startAVI(void) {
        ...
        vih->rcSource.left = 0;
        vih->rcSource.top = 0;
        vih->rcSource.right = cfg.width;
        vih->rcSource.bottom = cfg.height;
...
#define   JITTER_OFFSET            50000
    ...
    void endAVI(void) {
        ...
        if (pContext.input) { free(pContext.input); }
        pContext.input = NULL; //SAFEDELETE ( pContext.input );
        if (pContext.output) { free(pContext.output); }
        pContext.output = NULL; //SAFEDELETE ( pContext.output);
...
public:
    ...
    virtual bool __stdcall decStart(FOURCC fcc, double fps, unsigned int dx, unsigned int dy, const void *extradata, size_t extradata_len, uint64_t *csp) {
        ...
        vih->rcSource.left = 0;
        vih->rcSource.top = 0;
        vih->rcSource.right = dx;
        vih->rcSource.bottom = dy;
        vih->rcTarget = vih->rcSource;
