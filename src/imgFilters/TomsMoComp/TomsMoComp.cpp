...
public:
    ...
    virtual bool STDMETHODCALLTYPE create(bool isSSE2, bool isMMX2, bool is3dnow, bool _IsYUY2, int _TopFirst, int _Search_Effort, bool _Use_Vertical_Filter, int &width, int &height, stride_t srcStride[4], stride_t dstStride[4]) {
        ...
        src = new TVideoFrame;
        src->dx[0] = width;
        src->dx[1] = src->dx[2] = width / 2;
        src->stride[0] = srcStride[0];
        src->stride[1] = srcStride[1];
        src->stride[2] = srcStride[2];
        src->dy[0] = height;
        src->dy[1] = src->dy[2] = height / 2;
        dst = new TVideoFrame;
        memcpy(dst->dx, src->dx, sizeof(src->dx));
        memcpy(dst->dy, src->dy, sizeof(src->dy));
        dst->stride[0] = dstStride[0];
        dst->stride[1] = dstStride[1];
        dst->stride[2] = dstStride[2];
        prevSrc = new TVideoFrame;
        memcpy(prevSrc->dx, src->dx, sizeof(src->dx));
        memcpy(prevSrc->dy, src->dy, sizeof(src->dy));
        prevSrc->stride[0] = srcStride[0];
        prevSrc->stride[1] = srcStride[1];
        prevSrc->stride[2] = srcStride[2];
...
{
    strcpy(ver, "1.0.1.8, " COMPILER COMPILER_X64 COMPILER_INFO " (" __DATE__ " " __TIME__ ")");
