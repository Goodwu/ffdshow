...
public:
    ...
    unsigned char* GetWritePtr(int plane = 0) {
        return ptr[plane];
    }
    stride_t GetPitch(int plane = 0) {
        return stride[plane];
    }
    unsigned int GetRowSize(int plane = 0) {
        return Bpp * dx[plane];
    }
...
private:
    ...
    STDMETHODIMP_(void) getFrame(const unsigned char *cur[3], stride_t srcStride[3], unsigned char *dst[3], stride_t dstStride[3], int bobframe) { //hinting is done by caller
        if (bobframe == 0) {
            if (fields[fieldsptr]) {
                delete fields[fieldsptr];
            }
            fields[fieldsptr++] = new TVideoFrame(isYV12, width, height, cur, srcStride, scratchPitch, 0);
            if (fields[fieldsptr]) {
                delete fields[fieldsptr];
            }
            fields[fieldsptr++] = new TVideoFrame(isYV12, width, height, cur, srcStride, scratchPitch, 1);
            fieldstart += 2;
            if (fieldsptr == NFIELDS) {
                fieldsptr = 0;
            }
        }
        //if (!bob) n*=2;
    ...
    PVideoFrame fields[NFIELDS];
    int fieldstart, fieldsptr;
    ...
    template<bool YV12> void ShowMotionMask(unsigned char *dst[3], stride_t dstStride[3], int order, const KernelDeintMask *fullsizeMask) {
        ...
        for (int rowsLeft = height / 2; rowsLeft > 0; rowsLeft--) {
            ...
            for (int colsLeft = rowSize / sizeof(typename TshowMotionMaskTraits<YV12>::Tpixel); colsLeft > 0; colsLeft--) {
                ...
                *curDstPtr |= curMask;
                curMask &= TshowMotionMaskTraits<YV12>::mask2;
                *curDstPtr ^= curMask;
                curMaskPtr++;
                curDstPtr++;
            }
...
{
    strcpy(ver, "1.5.4, " COMPILER COMPILER_X64 COMPILER_INFO " (" __DATE__ " " __TIME__ ")");
